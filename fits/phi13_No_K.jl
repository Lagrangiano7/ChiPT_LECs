# Fitting phi13(phi2) --> p1, p2, p3, C0, C1
...
chisq(p, d) = sum( (d .- model(value.(phi2), value.(a2t0), value.(phi4), value.(fpik), p)) .^2 ./ (ADerrors.err.(phi13)).^2)
...
y = phi13./phik
ErrUtils.LoadErr(y)
PlotUtils.ScatterByEnsemble(beta, phi2, y, L"\phi_2", L"\phi_{13}/\phi_K", latexstring("\\mathrm{Single\\ fit\\ of}\\ \\phi_{13} \\mathrm{ \\ (no \\ K)}"), "phi13/phi13_No_K")

### Dumping details of params ###

ErrUtils.DumpErr(fitp, ["p1", "p2", "p3", "C'0", "C'1"], "phi13_No_K")