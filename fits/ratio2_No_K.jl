# Fitting phi12(phi2) --> p1, p2, p3, G0, G1
...
chisq(p, d) = sum( (d .- model(value.(phi2), value.(a2t0), value.(phi4), value.(fpik), p)) .^2 ./ (ADerrors.err.(ratio2)).^2)
...
for x in xaxis
        val = model_plot(x, 0, phi4[1], fpiks[length(fpiks)], fitp)
        push!(band, val)
    end
...
PlotUtils.ScatterByEnsemble(beta, phi2, ratio2, L"\phi_2", L"\frac{4\phi_{13}}{2\phi_4-\phi_2}+\frac{\phi_{12}}{\phi_2}", "Ratio2 (no K)", "ratio2/ratio2_No_K", true)

### Dumping details of params ###

ErrUtils.DumpErr(fitp, ["p1", "p2", "p3", "G0", "G1"], "ratio2_No_K")